...
//!
//! Path: Win `%APPDATA%/ezStreamer/`, Linux `~/.config/ezStreamer/`.
...
pub const CONFIG_DIR_NAME: &str = "ezStreamer";
...
mod tests {
    ...
    fn roundtrip_and_missing_file() {
        let dir = std::env::temp_dir().join(format!("ezstreamer-test-{}", std::process::id()));
    ...
    fn corrupt_file_falls_back_to_defaults() {
        let dir = std::env::temp_dir().join(format!("ezstreamer-test-corrupt-{}", std::process::id()));
        let path = dir.join(CONFIG_FILE_NAME);
