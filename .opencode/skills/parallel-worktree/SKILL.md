...
1. Create one worktree per session/branch (sibling of the repo root):
   `git worktree add ../ezStreamer-<suffix> -b <branch>`
   For an existing branch: `git worktree add ../ezStreamer-<suffix> <branch>`
...
4. Clean up after merge: `git worktree remove ../ezStreamer-<suffix>`,
