...
description: Ship an ezStreamer preview release (version bump, PR gate, tag, GitHub Release publish). Use when asked to release a preview version.
---
...
4. The tag-triggered `Release` workflow rebuilds the NSIS bundle with the
   pinned GStreamer subset and publishes the GitHub Release. Monitor to
   completion (see `ci-watch` skill).
...
```md
## ezStreamer preview.XX (preview.YY からの変更)
...
Windows-only preview build. GStreamer (LGPL) runtime subset is bundled — ...
```
...
- The GStreamer subset comes from the pinned MSVC runtime (`release.yml`
  `GSTREAMER_VERSION`). Bumping it is a deliberate change: update the pin,
  the `GSTREAMER-BUILD.txt` provenance, and `docs/design.md` §13.2 together.
