...
pub fn spawn_pipeline(
    ...
    // and aborts the UI thread (cannot unwind).
    apply_string_props(
        &aacenc,
    ...
    // connection cannot block `stop()` (and the worker loop) forever.
...
/// Encoder discovery for the UI (design §8.1). Registry lookup only: no child
/// processes, no test encodes, no cache file.
pub fn probe_encoder_infos() -> Vec<ezstreamer_core::ipc_types::EncoderInfo> {
    use ezstreamer_core::ipc_types::EncoderInfo;

    ensure_bundled_runtime();
    if ::gstreamer::init().is_err() {
        return vec![EncoderInfo {
            name: "libx264".into(),
            usable: false,
            reason: Some("GStreamer runtime not found".into()),
        }];
    }
    let mut infos = ezstreamer_core::gst::probe_with(has_element);
    // Diagnosis for "only libx264 usable" reports: record what the registry
    // actually exposes (bundle missing plugin files vs. no matching hardware,
...
/// Point GStreamer at the runtime bundled beside the exe (Release NSIS).
