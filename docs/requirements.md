# ezStreamer 要件定義書 v0.5

> 作成日: 2026-09-04 | 更新日: 2026-09-14 | 対象: 要件定義フェーズ | ステータス: Draft | リポジトリ: `ezStreamer`
...
> v0.4 (2026-09-14): **UI を OBS 風固定レイアウトへ再構成** (§7)。ステップレール (画面/音声/出力タブ)
> を廃止し「中央プレビューキャンバス + 右ミキサー/操作 + 下部3区画 + ステータスバー」に変更。
> プレビューを 5fps に改善 (F-SC-03)。**機能要件 ID と動作仕様は変更なし**。
> v0.5 (2026-09-14): **中段右を配信パネルへ集約** (ストリームキー / 視聴URLコピー / 配信CTA のみ)。
> 下段を [映像ソース 30% | 音量 40% | エンコーダ 30%] に再構成し、音声ミキサーを下段中央へ移動。
> エンコーダ区画はプロファイルComboBox + 詳細行に作り直し。**Ingest URL は設定画面へ移動** (F-URL-01 維持)。
...
| F-SC-03 | プレビュー | 配信前にローカルプレビュー(サムネ更新 5fps) | Must |
...
## 7. 画面遷移・UI要件 (v0.5: egui / OBS風固定レイアウト)
...
```
┌──────────────────────────────────────────────────────────────┐
│ ezStreamer │ ■配信中 1500 kbps │ ja/en │ 設定                 │
├───────────────────────────────────────────────┬──────────────┤
│                                               │ 配信:         │
│                                               │ Key [my-key_] │
│            プレビュー 16:9 (常時)              │ PC/Quest URL  │
│                                               ├──────────────┤
│                                               │ [ 配信開始 ]   │
├───────────────┬────────────────┬──────────────┴──────────────┤
│ 映像ソース     │ 音量: マスターVU │ エンコーダ:                  │
│ 全画面/ウィンドウ│ System/Apps    │ プロファイル [中 1280x720 ▼] │
│ + カーソル     │ per-app + mic  │ エンコーダ [自動▼] + 一覧     │
├───────────────┴────────────────┴─────────────────────────────┤
│ ■配信中 1500 kbps ドロップ 0                  バージョン …     │
└──────────────────────────────────────────────────────────────┘
```

- **原則:** OBSの「シーン/ソース」概念なし。チェックボックスとドロップダウンで完結。
  ただし配置はOBS風固定レイアウト (中央キャンバス + 右配信パネル + 下部設定区画)。
- **配信開始ボタンは常時表示** (中段右の配信パネル最下部に固定 / Stream CTA)。
  配信パネルは **ストリームキー / 視聴URLコピー / 配信ボタンのみ**。Ingest URL は設定画面。
  **VUメーター必須** (下段中央の音量区画)。
...
- 配信先: Ingest URL 編集 (既定 `rtmp://topaz.chat/live`)
