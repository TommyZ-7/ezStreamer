...
## 7. 画面遷移・UI要件 (v0.4: egui / OBS風固定レイアウト)
...
│                                               │ 音声: マスターVU│
│                                               │  System/Apps  │
│            プレビュー 16:9 (常時)              │  per-app + mic │
│                                               ├──────────────┤
│                                               │ [ 配信開始 ]   │
│                                               │ PC/Quest URL  │
├───────────────────────┬───────────────────────┴──────────────┤
│ 画面: 全画面/ウィンドウ │ 画質: [低][中][高][1080p]│ 配信先:      │
│ + カーソル (Portal)    │ + エンコーダ           │ Ingest / Key │
├───────────────────────┴──────────────────────────────────────┤
...
- **原則:** OBSの「シーン/ソース」概念なし。チェックボックスとドロップダウンで完結。
  ただし配置はOBS風固定レイアウト (中央キャンバス + 右ミキサー/操作 + 下部設定区画)。
- **配信開始ボタンは常時表示** (右サイドの操作パネル最下部に固定 / Controls)。
  **VUメーター必須**。
